...
% Export the Doppler-compensated and filtered beat signal

output_filename = 'beat_24GHz.mat';  % Unique name for 24 GHz radar
beat_signal = real(beat_filt);       % Save real part (or use abs(beat_filt) if desired)
fs_out = fs;                         % Save sampling rate
t_out = t_beat;                      % Time vector

% Save as .mat file for MATLAB use
save(output_filename, 'beat_signal', 'fs_out', 't_out');

% Optional: save as .txt for cross-platform usage
